...
class FirstMap extends Map{
  constructor(socketTable){
    var statics = [
      ...
      for(var i=0;i<150;i++){
        if(Math.random() > 0.5){
          statics.push(Static.getTreeData(Math.floor(Math.random() * 1150 + 50),Math.floor(Math.random() * 800 + 500)));
        }else{
          statics.push(Static.getTree2Data(Math.floor(Math.random() * 1500  + 50),Math.floor(Math.random() * 800 + 500)));
        }
      }
